...
from api.database import get_pending_jobs, update_job_status, get_job_full
...
async def _process_job(job: dict):
    ...
    try:
        update_job_status(job_id, JobStatus.PREPARING)

        with tempfile.TemporaryDirectory(prefix=f"lcbc_{job_id[:8]}_") as work_dir:
            # Download protein
            ...
            # Run Vina
            update_job_status(job_id, JobStatus.DOCKING)
